...
(defun symbol-name (symbol)
  ...
  (sys.int::%array-like-ref-t symbol sys.int::+symbol-name+))

(defun symbol-package (symbol)
  (sys.int::%type-check symbol sys.int::+object-tag-symbol+ 'symbol)
  (sys.int::%array-like-ref-t symbol sys.int::+symbol-package+))

(defun (setf symbol-package) (value symbol)
  (sys.int::%type-check symbol sys.int::+object-tag-symbol+ 'symbol)
  (setf (sys.int::%array-like-ref-t symbol sys.int::+symbol-package+) value))

(defun sys.int::symbol-global-value (symbol)
  (sys.int::%type-check symbol sys.int::+object-tag-symbol+ 'symbol)
  (let ((value (sys.int::%array-like-ref-t symbol sys.int::+symbol-value+)))
...
(defun (setf sys.int::symbol-global-value) (value symbol)
  (sys.int::%type-check symbol sys.int::+object-tag-symbol+ 'symbol)
  (setf (sys.int::%array-like-ref-t symbol sys.int::+symbol-value+) value))

(defun sys.int::%cas-symbol-global-value (symbol old new)
  ;; Should this check if the symbol is unbound as well?
  (sys.int::%type-check symbol sys.int::+object-tag-symbol+ 'symbol)
  (sys.int::%cas-array-like symbol sys.int::+symbol-value+ old new))

(defun symbol-plist (symbol)
  (sys.int::%type-check symbol sys.int::+object-tag-symbol+ 'symbol)
  (sys.int::%array-like-ref-t symbol sys.int::+symbol-plist+))

;; TODO: What kind of type-checking should be done here?
(defun (setf symbol-plist) (value symbol)
  (sys.int::%type-check symbol sys.int::+object-tag-symbol+ 'symbol)
  (setf (sys.int::%array-like-ref-t symbol sys.int::+symbol-plist+) value))
