  ...
  (cond ((sys.int::character-array-p thing)
        ...
        (t (debug-write-string "#<")
           ...
           (cond
             ...
             ((rw-lock-p thing)
              (debug-write-string " Rw-Lock ")
              (debug-write (rw-lock-name thing) (1+ depth))
              (debug-write-string " :State ")
              (debug-write (rw-lock-state thing) (1+ depth)))
             ((condition-variable-p thing)
              (debug-write-string " Condition-Variable ")
              (debug-write (wait-queue-name thing) (1+ depth)))
             ((event-p thing)
              (debug-write-string " Event ")
              (debug-write (wait-queue-name thing) (1+ depth)))
             ((wait-queue-p thing)
              (debug-write-string " Wait-Queue ")
              (debug-write (wait-queue-name thing) (1+ depth)))
             ((simple-irq-p thing)
              (debug-write-string " Simple-Irq :Number ")
              (debug-write (irq-platform-number (simple-irq-irq thing)) (1+ depth))
              (debug-write-string " :State ")
              (debug-write (simple-irq-state thing) (1+ depth))
              (debug-write-string " :Event-State ")
              (debug-write (event-state (simple-irq-event thing)) (1+ depth))
              (when (simple-irq-latch thing)
                (debug-write-string " :Latch ")
                (debug-write (simple-irq-latch thing) (1+ depth))))
             ((irq-p thing)
              (debug-write-string " Irq :Number ")
              (debug-write (irq-platform-number thing) (1+ depth))))
           (debug-write-string ">"))))
