...
(defun debug-print-line (&rest things)
  "Intel 8042-specific `debug-print-line' implementation.
Always return nil.  THINGS is a list of arguments to print."
  (apply #'sup::debug-print-line things)
  nil)
...
Return nil if a timeout is reached."
  (if (wait-until-input-is-available)
      (register-data-read)
      (debug-print-line "8042 result input wait failed.")))
...
INTERRUPT-FRAME and INTERRUPT-REQUEST-OBJECT are ignored."
  ...
  (let ((byte (register-data-read)))
    ...
    (case *intel-8042-debug-dump-state*
      ...
      (0 (cond ((eql byte
                     mezzano.supervisor.ps/2-keyboard::+scan-code-left-meta+)
                (setf *intel-8042-debug-dump-state* 1))))
      ;; Saw left-meta press.
      (1 (cond ((eql byte
                     mezzano.supervisor.ps/2-keyboard::+scan-code-left-meta+)
                #| Stay in this state. |#)
               ((eql byte mezzano.supervisor.ps/2-keyboard::+scan-code-f11+)
