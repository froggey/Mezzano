...
(defun cold-canonical-package-name (package)
  (cond ((or (string= package "CL")
             (string= package "COMMON-LISP"))
         "COMMON-LISP")
        ((or (string= package "SYS.INT")
             (string= package "SYSTEM.INTERNALS"))
         "SYSTEM.INTERNALS")
        ((string= package "SYSTEM") "SYSTEM")
        ((string= package "KEYWORD") "KEYWORD")
        (t (error "Unknown package ~S." package))))
(defun find-symbol (name &optional package)
  (setf package (cold-canonical-package-name (or package "SYSTEM.INTERNALS")))
  (let ((pkg *initial-obarray*)
        (package-keyword (cold-intern package "KEYWORD")))
    (dotimes (i (length pkg) (values nil nil))
      (when (and (eql (symbol-package (aref pkg i)) package-keyword)
                 (string= name (symbol-name (aref pkg i))))
  ...
  (let ((pkg *initial-obarray*)
    ...
    (dotimes (i (length pkg)
              (let ((sym (make-symbol (string name))))
                (setf (symbol-package sym) package-keyword)
