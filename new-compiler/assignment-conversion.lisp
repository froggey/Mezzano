  ...
  (cond ((and (typep (first form) 'constant)
        ...
        (t
         ...
         (let* ((needed-values '())
           ...
           (labels ((frob (vals)
                      (cond (vals
                             (list (make-instance 'constant :value '%contents)
                                   (! `(clambda (,(car (first vals))) ,(frob (rest vals))))
...
(defmethod convert-assignments ((form closure) &optional env)
  ...
  (let* ((set-args (remove-if-not (lambda (x) (getf (plist x) 'is-set))
                                  (append (closure-required-params* form)
                                          (mapcar 'first (closure-optional-params form))
                                          (mapcar 'second (closure-optional-params form))
                                          (when (closure-rest-param form)
                                            (list (closure-rest-param form)))
                                          (mapcar 'second (closure-keyword-params form))
                                          (mapcar 'third (closure-keyword-params form)))))
    ...
    (labels ((frob (args cells)
               (cond (args
                      (list (make-instance 'constant :value '%make-cell)
                            (! `(clambda (,(first cells))
                                  ,(frob (rest args) (rest cells))))
                            (first args)))
                     (t (convert-application-assignments (closure-body form)
                                                         (pairlis set-args cell-vars env))))))
      (copy-closure-with-new-body form (frob set-args cell-vars)))))

(defun copy-closure-with-new-body (closure new-body &optional ensure-continuation)
  (let ((new (make-instance 'closure
                            :required-params (closure-required-params* closure)
                            :optional-params (closure-optional-params closure)
                            :rest-param (closure-rest-param closure)
                            :keyword-params-enabled (closure-keyword-params-enabled closure)
                            :keyword-params (closure-keyword-params closure)
                            :allow-other-keywords (closure-allow-other-keywords closure)
                            :plist (plist closure)
                            :body new-body)))
    (when ensure-continuation
      (setf (getf (plist new) 'continuation) t))
    new))
