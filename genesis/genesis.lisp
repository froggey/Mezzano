...
(defun early-load (pathspec)
  ...
  (with-open-file (s pathspec)
	...
	((eql form s) t)
      ...
      (with-simple-restart (continue "Try evaluating ~S again." form)
        (genesis-eval form)))))
...
(defbuiltin proclaim (form)
  (cond ((eql (first form) (genesis-intern "SPECIAL"))
	   ...
	   (let ((symbol (resolve-function-name name)))
...
(define-forwarding-builtin %truncate truncate)
...
(define-forwarding-builtin floatp)
...
(defbuiltin #:%integer-as-single-float (value)
  (check-type value (unsigned-byte 32))
  #+sbcl (if (logtest value #x80000000)
             (sb-kernel:make-single-float (logior value -2147483648))
             (sb-kernel:make-single-float value))
  #-(or sbcl) (error "Not implemented on this platform!"))

(defbuiltin #:%single-float-as-integer (value)
