...
(defmethod file-properties-using-host ((host fat-host) path)
  (multiple-value-bind (parent-dir parent-cluster file-offset) (open-file-metadata host path)
    ...
    (let ((time (read-write-time parent-dir file-offset))
      ...
      (list :write-date (encode-universal-time (ash (ldb (byte 5 0) time) 1)
                                               (ldb (byte 6 5) time)
                                               (ldb (byte 5 11) time)
                                               (ldb (byte 5 0) date)
                                               (ldb (byte 4 5) date)
...
(defmethod set-file-properties-using-host ((host fat-host) path &key write-date)
  (error "TODO: Actually implement this..."))
