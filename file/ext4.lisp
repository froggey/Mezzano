...
(defstruct superblock
  ...
  (encoding nil :type (unsigned-byte 16))
  (encoding-flags nil :type (unsigned-byte 16)))
    ...
    (let* ((superblock (block-device-read-sector disk 2 2))
      ...
      (when (and (= magic #xEF53) (check-feature-incompat feature-incompat))
        (make-superblock :inodes-count (sys.int::ub32ref/le superblock 0)
                         :blocks-count (dpb (sys.int::ub32ref/le superblock 336)
                                            (byte 32 32)
                                            (sys.int::ub32ref/le superblock 4))
                         :r-blocks-count (dpb (sys.int::ub32ref/le superblock 340)
                                              (byte 32 32)
                                              (sys.int::ub32ref/le superblock 8))
                         :free-blocks-count (dpb (sys.int::ub32ref/le superblock 344)
                                                 (byte 32 32)
                         ...
                         :hash-seed (make-array '(4) :element-type '(unsigned-byte 32)
                                                :initial-contents (loop :for i :from 236 :to 248 :by 4
                         ...
                         :encoding (sys.int::ub16ref/le superblock 636)
                         :encoding-flags (sys.int::ub16ref/le superblock 638))))))
...
(defun read-block-group-descriptor (superblock block offset)
  (if (logbitp +incompat-64bit+ (superblock-feature-incompat superblock))
      (make-block-group-descriptor :block-bitmap (dpb (sys.int::ub32ref/le block (+ 32 offset))
                                                      (byte 32 32)
                                                      (sys.int::ub32ref/le block (+ 0 offset)))
                                   :inode-bitmap (dpb (sys.int::ub32ref/le block (+ 36 offset))
                                                      (byte 32 32)
                                                      (sys.int::ub32ref/le block (+ 4 offset)))
                                   :inode-table (dpb (sys.int::ub32ref/le block (+ 40 offset))
                                                     (byte 32 32)
                                                     (sys.int::ub32ref/le block (+ 8 offset)))
                                   :free-blocks-count (dpb (sys.int::ub16ref/le block (+ 44 offset))
                                                           (byte 16 16)
                                                           (sys.int::ub16ref/le block (+ 12 offset)))
                                   :free-inodes-count (dpb (sys.int::ub16ref/le block (+ 46 offset))
                                                           (byte 16 16)
                                                           (sys.int::ub16ref/le block (+ 14 offset)))
                                   :used-dirs-count (dpb (sys.int::ub16ref/le block (+ 48 offset))
                                                         (byte 16 16)
                                                         (sys.int::ub16ref/le block (+ 16 offset)))
                                   :flags (sys.int::ub16ref/le block (+ 18 offset))
                                   :exclude-bitmap (dpb (sys.int::ub32ref/le block (+ 52 offset))
                                                        (byte 32 32)
                                                        (sys.int::ub32ref/le block (+ 20 offset)))
                                   :block-bitmap-csum (dpb (sys.int::ub16ref/le block (+ 56 offset))
                                                           (byte 16 16)
                                                           (sys.int::ub16ref/le block (+ 24 offset)))
                                   :inode-bitmap-csum (dpb (sys.int::ub16ref/le block (+ 58 offset))
                                                           (byte 16 16)
                                                           (sys.int::ub16ref/le block (+ 26 offset)))
                                   :itable-unused (dpb (sys.int::ub16ref/le block (+ 50 offset))
                                                       (byte 16 16)
      ...
      (make-block-group-descriptor :block-bitmap (sys.int::ub32ref/le block (+ 0 offset))
                                   ...
                                   :flags (sys.int::ub16ref/le block (+ 18 offset))
                                   :exclude-bitmap (sys.int::ub32ref/le block (+ 20 offset))
                                   :block-bitmap-csum (sys.int::ub16ref/le block (+ 24 offset))
                                   :inode-bitmap-csum (sys.int::ub16ref/le block (+ 26 offset))
                                   :itable-unused (sys.int::ub16ref/le block (+ 28 offset)))))
