...
(defun keyboard-forwarder-process-one-key ()
  (let ((byte (mezzano.supervisor:ps/2-read-keyboard)))
    (cond
      ((eql byte +extended-scan-code+)
       ;; Reading extended scan code.
       (setf byte (mezzano.supervisor:ps/2-read-keyboard))
       (cond ((eql byte #x2A)
              ;; Start of Print Screen's make code.
              (when (eql (mezzano.supervisor:ps/2-read-keyboard) #xE0)
                (when (eql (mezzano.supervisor:ps/2-read-keyboard) #x37)
             ...
             ((eql byte #xB7)
              ...
              (when (eql (mezzano.supervisor:ps/2-read-keyboard) #xE0)
                (when (eql (mezzano.supervisor:ps/2-read-keyboard) #xAA)
      ...
      ((eql byte #xE1)
       ...
       (when (eql (mezzano.supervisor:ps/2-read-keyboard) #x1D)
         (when (eql (mezzano.supervisor:ps/2-read-keyboard) #x45)
           (when (eql (mezzano.supervisor:ps/2-read-keyboard) #xE1)
             (when (eql (mezzano.supervisor:ps/2-read-keyboard) #x9D)
               (when (eql (mezzano.supervisor:ps/2-read-keyboard) #xC5)
                 ;; Pause/Break pressed.
                 ;; E1 1D 45 E1 9D C5
...
(defun mouse-forwarder-thread ()
  ...
  (loop
     (mezzano.internals::log-and-ignore-errors
       (let ((byte-1 (mezzano.supervisor:ps/2-read-auxiliary)))
         ;; Check sync bit.
         (when (logtest byte-1 #b00001000)
           (let* ((byte-2 (mezzano.supervisor:ps/2-read-auxiliary))
                  (byte-3 (mezzano.supervisor:ps/2-read-auxiliary))
                  (byte-4 (if (eql
                               mezzano.supervisor:*ps/2-mouse-device-identifier*
                               +ps/2-intellimouse-id+)
                              (mezzano.supervisor:ps/2-read-auxiliary)
                              0))
