    ...
    (with-open-stream (stream stream)
      (let ((line (read-line stream nil)))
        ...
        (multiple-value-bind (request path get-parameters)
          ...
          (format t "HTTP: ~S ~S~%" request path)
          (cond ((string-equal "GET" request)
                 (cond ((eql path '())
                        (send stream "HTTP/1.0 200 OK~%")
                        (send stream "Content-Type: text/html; charset=utf-8~%~%")
                        (send stream "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"~%")
                        (send stream "\"http://www.w3.org/TR/html4/loose.dtd\">~%")
                        (send stream "<HTML><HEAD><TITLE>(LISP OS)</TITLE></HEAD>~%")
                        (send stream "<BODY><H1>Hello, World!</H1><BR>~%")
                        (let ((*standard-output* stream))
                          (declare (special *standard-output* *terminal-io*))
                          (room))
                        (send stream "<BR>~%")
                        #+nil (send stream "<BR>~%Running on ~A ~S<BR>~%"
                                      (lisp-implementation-type)
                                      (lisp-implementation-version))
                        (send stream "<IMG SRC=\"/logo300x100.jpg\" ALT=\"Made with Lisp.\">~%")
                        (send stream "</BODY></HTML>~%"))
