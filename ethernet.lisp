          ...
          (cond ((string-equal "GET" request)
                 (cond ((eql path '())
                        (with-output-to-string (s)
                          ...
                          (format s "Content-Type: text/html; charset=utf-8~%~%")
                          (format s "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.01 Transitional//EN\"~%")
                          (format s "\"http://www.w3.org/TR/html4/loose.dtd\">~%")
                          (format s "<HTML><HEAD><TITLE>(LISP OS)</TITLE></HEAD>~%")
                          (format s "<BODY><H1>Hello, World!</H1><BR>~%")
                          (let ((*standard-output* s))
                            (declare (special *standard-output* *terminal-io*))
                            (room))
                          (format s "<BR>~%")
                          #+nil (format s "<BR>~%Running on ~A ~S<BR>~%"
                                      (lisp-implementation-type)
                                      (lisp-implementation-version))
                          (format s "<IMG SRC=\"/logo300x100.jpg\" ALT=\"Made with Lisp.\">~%")
                          (format s "</BODY></HTML>~%")
