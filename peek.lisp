...
(defun peek-network ()
  ...
  (dolist (route sys.net::*routing-table*)
    ...
    ;; (list nil gateway netmask interface) default
    ;; (list (logand local-ip netmask) nil netmask interface)
    (write-char #\Space)
    (if (first route)
        (sys.net::format-tcp4-address nil (first route))
        (write-string ":DEFAULT"))
    (write-char #\Space)
    (if (second route)
        (sys.net::format-tcp4-address nil (second route))
        (write-string "N/A"))
