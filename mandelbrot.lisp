...
(defun render-mandelbrot (framebuffer x-offset y-offset width height)
  (let ((scale (/ 4 width))
      ...
      (dotimes (x width)
        (let ((r 0) (g 0) (b 0))
          ...
          (setf r (/ r 4)
                g (/ g 4)
                g (/ b 4))
          (setf (aref framebuffer (+ y y-offset) (+ x x-offset)) (sys.graphics::make-colour (list r g b))))))))
...
(defmethod sys.graphics::window-redraw ((window mandelbrot-window))
  (let* ((fb (sys.graphics::window-backbuffer window))
    ...
    (multiple-value-bind (left right top bottom)
      ...
      (let ((p (sys.int::make-process "Mandelbrot renderer")))
        (sys.int::process-preset p #'render-mandelbrot fb left top width height)
        (sys.int::process-enable p)))))
