      ...
      (let* ((elements-per-word (/ 64 element-size))
        ...
        (when (not (eql element-type 't))
          ...
          (dotimes (i total-data-words)
            (let ((value 0))
              (dotimes (j element-size)
                (when (< (+ (* i elements-per-word) j) n-elements)
                  (setf (ldb (byte element-size elements-per-word) value) (row-major-aref object (+ (* i elements-per-word) j))))
...
(defmethod allocate-object ((object env:structure-slot-definition) image environment)
  ...
  (allocate 10 image :wired sys.int::+tag-object+))
...
(defmethod initialize-object ((object env:structure-slot-definition) value image environment)
  ...
  (setf (object-slot image value 5)
        (serialize-object (env:structure-slot-definition-ref-fn object) image environment))
  (setf (object-slot image value 6)
        (serialize-object (env:structure-slot-definition-index object) image environment))
  (setf (object-slot image value 7)
        (serialize-object (env:structure-slot-definition-fixed-vector object) image environment))
  (setf (object-slot image value 8)
...
(defmethod initialize-object ((object env:instance-object) value image environment)
  (let ((sdef (env:instance-structure-definition object)))
    ...
    (dolist (slot (env:structure-definition-slots sdef))
      (let ((ref-fn (env:structure-slot-definition-ref-fn slot))
            (index (env:structure-slot-definition-index slot)))
        (cond ((env:structure-slot-definition-fixed-vector slot)
               (dotimes (i (env:structure-slot-definition-fixed-vector slot))
                 (let ((slot-value (aref (slot-value object (env:structure-slot-definition-name slot)) i)))
                   (cond ((eql ref-fn (env:translate-symbol environment 'sys.int::%object-ref-t))
                          (setf (object-slot image value (+ index i))
                                (serialize-object slot-value image environment)))
                         ((eql ref-fn (env:translate-symbol environment 'sys.int::%object-ref-unsigned-byte-64-unscaled))
                          (setf (object-slot image value (/ (+ index (* i 8)) 8)) slot-value))
                         ((eql ref-fn (env:translate-symbol environment 'sys.int::%object-ref-signed-byte-64-unscaled))
                          (setf (object-slot image value (/ (+ index (* i 8)) 8)) (ldb (byte 64 0) slot-value)))
                         ((eql ref-fn (env:translate-symbol environment 'sys.int::%object-ref-unsigned-byte-32-unscaled))
                          (multiple-value-bind (slot-index slot-offset)
                              (truncate (+ index (* i 4)) 2)
                            (setf (ldb (byte 32 (* slot-offset 32))
                                       (object-slot image value (/ slot-index 4)))
                                  slot-value)))
                         ((eql ref-fn (env:translate-symbol environment 'sys.int::%object-ref-unsigned-byte-8-unscaled))
                          (multiple-value-bind (slot-index slot-offset)
                              (truncate (+ index i) 8)
                            (setf (ldb (byte 8 (* slot-offset 8))
                                       (object-slot image value slot-index))
                                  slot-value)))
                         (t (error "unsupported instance ref fn ~S" ref-fn))))))
              (t
               (let ((slot-value (slot-value object (env:structure-slot-definition-name slot))))
                 (cond ((eql ref-fn (env:intern environment '#:%object-ref-t :system.internals))
                        (setf (object-slot image value index)
                              (serialize-object slot-value image environment)))
                       ((eql ref-fn (env:translate-symbol environment 'sys.int::%object-ref-unsigned-byte-64-unscaled))
                        (setf (object-slot image value (/ index 8)) slot-value))
                       ((eql ref-fn (env:translate-symbol environment 'sys.int::%object-ref-signed-byte-64-unscaled))
                        (setf (object-slot image value (/ index 8)) (ldb (byte 64 0) slot-value)))
                       ((eql ref-fn (env:translate-symbol environment 'sys.int::%object-ref-unsigned-byte-32-unscaled))
                        (multiple-value-bind (slot-index slot-offset)
                            (truncate index 2)
                          (setf (ldb (byte 32 (* slot-offset 32))
                       ...
                       ((eql ref-fn (env:translate-symbol environment 'sys.int::%object-ref-unsigned-byte-8-unscaled))
                        (multiple-value-bind (slot-index slot-offset)
                          ...
                          (setf (ldb (byte 8 (* slot-offset 8))
                                ...
                                slot-value)))
                       (t
                        (error "unsupported instance ref fn ~S" ref-fn))))))))))
