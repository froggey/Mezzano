...
(defmacro define-u-b-object-ref-unscaled (name width read-op write-op)
  ...
  `(progn
     (defbuiltin ,name (object offset) ()
       (let ((constant-offset (and (constant-type-p offset `(signed-byte ,(- 64 sys.int::+n-fixnum-bits+)))
         ...
         ;; Read.
         (cond (constant-offset
                (let ((disp (object-slot-displacement constant-offset 1)))
                  (cond ((or (<= -256 disp 255)
                             (and (<= 0 disp 16380)
                                  (zerop (logand disp #b111))))
                         (emit `(,',read-op :w9 (:x1 ,disp))))
...
(define-u-b-object-ref-unscaled sys.int::%%object-ref-unsigned-byte-8-unscaled  1 lap:ldrb lap:strb)
(define-u-b-object-ref-unscaled sys.int::%%object-ref-unsigned-byte-16-unscaled 2 lap:ldrh lap:strh)
(define-u-b-object-ref-unscaled sys.int::%%object-ref-unsigned-byte-32-unscaled 4 lap:ldr  lap:str)
...
(defbuiltin sys.int::%object-ref-unsigned-byte-64-unscaled (object offset) ()
  (let ((constant-offset (and (constant-type-p offset `(signed-byte ,(- 64 sys.int::+n-fixnum-bits+)))
    ...
    ;; Read.
    (cond (constant-offset
           (emit-object-load :x10 :x0 :slot constant-offset :scale 1))
          (t
           (emit `(lap:sub :x9 :x9 ,(- (+ 8 (- sys.int::+tag-object+))))
                 `(lap:ldr :x10 (:x0 :x9)))))
    (box-unsigned-byte-64-x10)
...
(defbuiltin (setf sys.int::%object-ref-unsigned-byte-64-unscaled) (new-value object offset) ()
  (let ((type-error-label (gensym))
    ...
    ;; Write.
    (cond (constant-offset
           (emit-object-store :x10 :x1 :slot constant-offset :scale 1))
          (t
           (emit `(lap:sub :x9 :x9 ,(- (+ 8 (- sys.int::+tag-object+))))
                 `(lap:str :x10 (:x1 :x9)))))
    *x0-value*))

(defmacro define-s-b-object-ref-unscaled (name width read-op write-op)
  (declare (ignore width))
  `(progn
     (defbuiltin ,name (object offset) ()
       (let ((constant-offset (and (constant-type-p offset `(signed-byte ,(- 64 sys.int::+n-fixnum-bits+)))
...
(define-s-b-object-ref sys.int::%%object-ref-signed-byte-8-unscaled  1 lap:ldrsb lap:strb)
(define-s-b-object-ref sys.int::%%object-ref-signed-byte-16-unscaled 2 lap:ldrsh lap:strh)
(define-s-b-object-ref sys.int::%%object-ref-signed-byte-32-unscaled 4 lap:ldrsw lap:str)
