...
(defgeneric flush-form (form)
  (:documentation "Kill off a form and reduce any use-counts it was holding."))

(defmethod flush-form ((form ast-block))
  (flush-form (body form)))

(defmethod flush-form ((form ast-function))
  nil)

(defmethod flush-form ((form ast-go))
  (decf (go-tag-use-count (target form)))
  (decf (lexical-variable-use-count (go-tag-tagbody (target form)))))

(defmethod flush-form ((form ast-if))
  (flush-form (test form))
  (flush-form (if-then form))
  (flush-form (if-else form)))

(defmethod flush-form ((form ast-let))
  (loop
     for (variable init-form) in (bindings form)
     do (flush-form init-form))
  (flush-form (body form)))

(defmethod flush-form ((form ast-multiple-value-bind))
  (flush-form (value-form form))
  (flush-form (body form)))

(defmethod flush-form ((form ast-multiple-value-call))
  (flush-form (function-form form))
  (flush-form (value-form form)))

(defmethod flush-form ((form ast-multiple-value-prog1))
  (flush-form (value-form form))
  (flush-form (body form)))

(defmethod flush-form ((form ast-progn))
  (dolist (i (forms form))
    (flush-form i)))

(defmethod flush-form ((form ast-quote))
  nil)

(defmethod flush-form ((form ast-return-from))
  (decf (lexical-variable-use-count (target form)))
  (flush-form (value form))
  (flush-form (info form)))

(defmethod flush-form ((form ast-setq))
  (decf (lexical-variable-use-count (setq-variable form)))
  (decf (lexical-variable-write-count (setq-variable form)))
  (flush-form (value form)))

(defmethod flush-form ((form ast-tagbody))
  (dolist (i (statements form))
    (unless (go-tag-p i)
      (flush-form i))))

(defmethod flush-form ((form ast-the))
  (flush-form (value form)))

(defmethod flush-form ((form ast-unwind-protect))
  (flush-form (protected-form form))
  (flush-form (cleanup-function form)))

(defmethod flush-form ((form ast-call))
  (mapc #'flush-form (arguments form)))

(defmethod flush-form ((form ast-jump-table))
  (flush-form (value form))
  (mapc #'flush-form (targets form)))

(defmethod flush-form ((form lexical-variable))
  (decf (lexical-variable-use-count form)))

(defmethod flush-form ((form lambda-information))
