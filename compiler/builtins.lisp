  ...
  (let ((full-logior (gensym))
    ...
    (emit-trailer (full-logior)
      ...
      (emit `(sys.lap-x86:mov32 :ecx 16)
            `(sys.lap-x86:mov64 :r13 (:constant sys.int::generic-logior))
            `(sys.lap-x86:call (:symbol-function :r13))
  ...
  (let ((full-logxor (gensym))
    ...
    (emit-trailer (full-logxor)
      (when (constant-type-p x 'fixnum)
        (load-constant :r9 (second x)))
      (emit `(sys.lap-x86:mov32 :ecx 16)
            `(sys.lap-x86:mov64 :r13 (:constant sys.int::generic-logxor))
            `(sys.lap-x86:call (:symbol-function :r13))
            `(sys.lap-x86:mov64 :lsp :rbx)
            `(sys.lap-x86:jmp ,resume)))
    (cond ((constant-type-p x 'fixnum)
...
(defbuiltin lognot (integer) ()
  (let ((not-fixnum (gensym "lognot-other"))
    ...
    (emit-trailer (not-fixnum)
      (emit `(sys.lap-x86:mov32 :ecx 8)
            `(sys.lap-x86:mov64 :r13 (:constant sys.int::generic-lognot))
            `(sys.lap-x86:call (:symbol-function :r13))
            `(sys.lap-x86:mov64 :lsp :rbx)
            `(sys.lap-x86:jmp ,resume)))
        ...
        (t (let ((done-label (gensym))
             ...
             (emit-trailer (ovfl)
               (emit
                ...
                `(sys.lap-x86:mov64 :r13 (:constant sys.int::%ash))
                `(sys.lap-x86:call (:symbol-function :r13))
                `(sys.lap-x86:mov64 :lsp :rbx)
...
(defbuiltin apply (function arguments) (nil)
  (let ((type-error-label (gensym))
    ...
    (emit `(sys.lap-x86:xor32 :ecx :ecx)
          ...
          `(sys.lap-x86:cmp8 :al ,sys.int::+tag-cons+)
          `(sys.lap-x86:jne ,type-error-label))
    ;; Adjusting the stack while unpacking the list.
    (adjust-lisp-stack 1)
    (emit `(sys.lap-x86:mov64 :r9 (:car :r8))
    ...
    (adjust-lisp-stack -1)
    (emit `(sys.lap-x86:cmp64 :rcx ,(fixnum-to-raw 1))
          `(sys.lap-x86:je ,arg-done)
          `(sys.lap-x86:mov64 :r9 (:lsp)))
    (adjust-lisp-stack -1)
    (emit `(sys.lap-x86:cmp64 :rcx ,(fixnum-to-raw 2))
          `(sys.lap-x86:je ,arg-done)
          `(sys.lap-x86:mov64 :r10 (:lsp)))
    (adjust-lisp-stack -1)
    (emit `(sys.lap-x86:cmp64 :rcx ,(fixnum-to-raw 3))
          `(sys.lap-x86:je ,arg-done)
          `(sys.lap-x86:mov64 :r11 (:lsp)))
    (adjust-lisp-stack -1)
    (emit `(sys.lap-x86:cmp64 :rcx ,(fixnum-to-raw 4))
          `(sys.lap-x86:je ,arg-done)
