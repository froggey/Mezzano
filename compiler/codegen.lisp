...
(defun bind (sym tag)
  (let ((slot (find-stack-slot)))
    (setf (aref *stack-values* slot) (cons sym :home))
    (push (list sym slot) *special-bindings*)
    (smash-r8)
    (load-constant :r9 sym)
    ;; Save old value.
    (emit `(sys.lap-x86:mov64 :r8 (:symbol-value :r9))
...
(defun unbind-to (bindings target)
  (unless (eql bindings target)
    ...
    (let ((b (first bindings)))
      (cond ((null (cdr b))
             ;; Unwind-protect cleanup function.
             (case *for-value*
               (:multiple
             ...
             (case *for-value*
               ...
               (:multiple
                ...
                (emit `(sys.lap-x86:mov64 (:lsp -8) nil)
                      ...
                      `(sys.lap-x86:mov64 (:lsp 8) :r9))
                ;; Unbind.
                (load-constant :r9 (first b))
                (emit `(sys.lap-x86:mov64 :r8 (:stack ,(second b)))
                      `(sys.lap-x86:mov64 (:symbol-value :r9) :r8))
                ;; Restore registers.
                (emit `(sys.lap-x86:mov64 :r8 (:lsp 0))
                      `(sys.lap-x86:mov64 :r9 (:lsp 8))
                      `(sys.lap-x86:add64 :lsp 16)))
               (t (load-constant :r9 (first b))
                  (emit `(sys.lap-x86:mov64 :r10 (:stack ,(second b)))
...
(defun cg-unwind-protect (form)
  ...
  (let* ((cleanup-tag (let ((*for-value* t))
                        (cg-form (third form))))
         (*special-bindings* (cons (list cleanup-tag) *special-bindings*)))
