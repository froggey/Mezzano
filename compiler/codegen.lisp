...
(defun flush-data-registers (multiple-values)
  ...
  (if multiple-values
      (emit `(sys.lap-x86:cmp32 :ecx ,(fixnum-to-raw 2))
            `(sys.lap-x86:cmov32b :r9d :ebx)
            `(sys.lap-x86:cmp32 :ecx ,(fixnum-to-raw 3))
            `(sys.lap-x86:cmov32b :r10d :ebx)
            `(sys.lap-x86:cmp32 :ecx ,(fixnum-to-raw 4))
            `(sys.lap-x86:cmov32b :r11d :ebx)
            `(sys.lap-x86:cmp32 :ecx ,(fixnum-to-raw 5))
            `(sys.lap-x86:cmov32b :r12d :ebx))
	  ...
	  (t (when (prep-arguments-for-call (rest form))
	       ...
	       (load-constant :rcx (length (rest form)))
               (cond ((can-tail-call (rest form))
                      (emit-tail-call (object-ea :r13 :slot +symbol-function+) (first form))
                      nil)
                     (t (emit `(sys.lap-x86:call ,(object-ea :r13 :slot +symbol-function+)))
                        (if (member *for-value* '(:multiple :tail))
                            (emit-gc-info :multiple-values 0)
