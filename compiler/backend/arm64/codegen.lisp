...
(defun layout-local-variables (backend-function)
  (let ((worklist (list (cons (ir:first-instruction backend-function) '())))
    ...
    (loop
       ...
       (let* ((entry (pop worklist))
         ...
         (typecase inst
           (ir:bind-local-instruction
            (setf (gethash inst layout) (length stack))
            (push inst stack))
           (ir:unbind-local-instruction
            (assert (eql (ir:unbind-local-local inst)
                         (first stack)))
            (pop stack)))
         (dolist (next (ir:successors backend-function inst))
  ...
  (let* ((local-layout (layout-local-variables backend-function))
         (layout (loop
                    ...
                    when (not (and (not (endp vreg-defs))
                                   (endp (rest vreg-defs))
                                   (typep (first vreg-defs) 'ir:argument-setup-instruction)
      ...
      (loop
         ...
         do
           (when (listp entry)
             (loop
                for local in entry
                for ast = (ir:bind-local-ast local)
...
(defun vreg-stack-slot (vreg)
  ...
  (or (position vreg *stack-layout*)
      (error "Missing stack slot for vreg ~S" vreg)))

(defun local-variable-stack-slot (binding)
  (check-type binding ir:bind-local-instruction)
  (or (position-if (lambda (slot)
                     (and (listp slot)
                          (member binding slot)))
                   *stack-layout*)
        ...
        (let ((*emitted-lap* '())
              (*current-frame-layout* (coerce (loop
                                                 ...
                                                 collect (if (or (eql elt :raw)
                                                                 (and (typep elt 'ir:virtual-register)
                                                                      (not (eql (ir:virtual-register-kind elt) :value))))
                                                             0
                                                             1))
                                              'simple-bit-vector))
    ...
    (flet ((emit-arg-error ()
      ...
      (cond ((ir:argument-setup-rest instruction)
             ...
             (when (ir:argument-setup-required instruction)
               ...
               (emit `(lap:subs :xzr :x5 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw
            ...
            ((and (ir:argument-setup-required instruction)
             ...
             ;; A range.
             (emit `(lap:sub :x9 :x5 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw
                                       (length (ir:argument-setup-required instruction))))
                   `(lap:subs :xzr :x9 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw
...
(defun emit-dx-rest-list (argument-setup)
  (let* ((regular-argument-count (+ (length (ir:argument-setup-required argument-setup))
    ...
    (cond ((zerop regular-argument-count)
          ...
          (t
           (emit `(lap:subs :x5 :x5 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw regular-argument-count)))
    ...
    (emit `(lap:add :x10 :x10 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw 2)))
    ...
    (emit `(lap:add :x10 :x5 :x5)
          `(lap:add :x10 :x10 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw 1)))
    ...
    (emit `(lap:subs :x10 :x10 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw 1)))
    ...
    (loop
       ...
       do (emit `(lap:str ,reg (:post :x12 16))
                `(lap:subs :x5 :x5 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw 1))
    ...
    (emit `(lap:subs :x5 :x5 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw 1)))
...
(defmethod emit-lap (backend-function (instruction ir:bind-local-instruction) uses defs)
  (emit-stack-store (ir:bind-local-value instruction) (local-variable-stack-slot instruction)))

(defmethod emit-lap (backend-function (instruction ir:unbind-local-instruction) uses defs)
  nil)

(defmethod emit-lap (backend-function (instruction ir:load-local-instruction) uses defs)
  (emit-stack-load (ir:load-local-destination instruction)
                   (local-variable-stack-slot (ir:load-local-local instruction))))

(defmethod emit-lap (backend-function (instruction ir:store-local-instruction) uses defs)
  (emit-stack-store (ir:store-local-value instruction)
                    (local-variable-stack-slot (ir:store-local-local instruction))))
...
(defmethod emit-lap (backend-function (instruction ir:constant-instruction) uses defs)
  (let ((value (ir:constant-value instruction))
    ...
    (cond ((typep value 'ir:backend-function)
           (emit `(lap:ldr ,dest (:constant ,(compile-backend-function value *target*)))))
          ((sys.c::fixnump value)
           (load-literal dest (mezzano.compiler.codegen.x86-64::fixnum-to-raw value)))
          ((characterp value)
           (load-literal dest (mezzano.compiler.codegen.x86-64::character-to-raw value)))
          ((eql value 'nil)
...
(defmethod emit-lap (backend-function (instruction ir:return-instruction) uses defs)
  (load-literal :x5 (mezzano.compiler.codegen.x86-64::fixnum-to-raw 1))
...
(defun call-argument-setup (call-arguments)
  (let* ((stack-args (nthcdr 5 call-arguments))
    ...
    (load-literal :x5 (mezzano.compiler.codegen.x86-64::fixnum-to-raw n-args))))
...
(defmethod emit-lap (backend-function (instruction ir:save-multiple-instruction) uses defs)
  (let* ((save-data (gethash instruction *saved-multiple-values*))
    ...
    (emit `(lap:add :x9 :x5 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw 2)))
    (emit `(lap:and :x9 :x9 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw (lognot 1))))
    ;; Save SP.
    ...
    (let ((clear-loop-head (gensym "MVP1-CLEAR-LOOP"))
      ...
      (emit `(lap:sub :x10 :x10 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw 1)))
    ...
    (loop
       ...
       for offset from 0
       do
         (emit `(lap:subs :xzr :x5 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw offset)))
         (emit `(lap:b.le ,save-done))
       ;; 1+ to skip header.
         (emit `(lap:str ,reg (:sp ,(* (1+ offset) 8)))))
    ;; Save values in the MV area.
    ;; Number of values remaining.
    (emit `(lap:subs :x9 :x5 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw 5)))
    (emit `(lap:b.le ,save-done))
    ...
    (emit `(lap:subs :x9 :x9 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw 1)))
...
(defmethod emit-lap (backend-function (instruction ir:multiple-value-bind-instruction) uses defs)
  (loop
     ...
     do
       (emit `(lap:subs :xzr :x5 ,(mezzano.compiler.codegen.x86-64::fixnum-to-raw i)))
       (cond (regs
              (let ((reg (pop regs)))
...
(defmethod emit-lap (backend-function (instruction ir:values-instruction) uses defs)
  (cond ((endp (ir:values-values instruction))
         ...
         (load-literal :x5 0))
        (t
         (load-literal :x5 (mezzano.compiler.codegen.x86-64::fixnum-to-raw (min 5 (length (ir:values-values instruction)))))
         (loop
