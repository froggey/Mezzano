  ...
  (let ((start-bytes-consed mezzano.runtime::*bytes-consed*)
    ...
    (multiple-value-prog1 (funcall fn)
      (let ((finish-cycle (tsc))
            ...
            (finish-pager mezzano.supervisor::*pager-meter*)
        ...
        (let ((pager-time (- finish-pager start-pager)))
          (when (plusp pager-time)
            (format *trace-output* "; Pager ran for ~:D seconds.~%" (float (/ pager-time internal-time-units-per-second)))))
