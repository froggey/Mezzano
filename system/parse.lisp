  ...
  (let ((state :required)
    ...
    (dolist (i lambda-list)
      (ecase state
        ...
        (:rest (case i
                 ((&optional &rest &key &allow-other-keys &aux &closure &count)
                  (error "Unexpected ~S in lambda list ~S" i lambda-list))
    ...
    (values (nreverse required) (nreverse optional) rest enable-keys (nreverse keys) allow-other-keys (nreverse aux) closure count)))
