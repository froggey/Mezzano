...
(defclass synonym-stream (sys.gray:fundamental-stream)
...
(defmethod sys.gray:stream-file-position ((stream synonym-stream) &optional (position-spec nil position-spec-p))
  (if position-spec-p
      (file-position (follow-synonym-stream stream) position-spec)
      (file-position (follow-synonym-stream stream))))

(defmethod sys.gray:stream-file-length ((stream synonym-stream))
  (file-length (follow-synonym-stream stream)))

(defmethod sys.gray:stream-file-string-length ((stream synonym-stream) string)
  (file-string-length (follow-synonym-stream stream) string))

(defmethod sys.gray:stream-clear-input ((stream synonym-stream))
  (clear-input (follow-synonym-stream stream)))

(defmethod sys.gray:stream-read-sequence ((stream synonym-stream) seq &optional (start 0) end)
  (read-sequence seq (follow-synonym-stream stream) :start start :end end))

(defmethod sys.gray:stream-clear-output ((stream synonym-stream))
  (clear-output (follow-synonym-stream stream)))

(defmethod sys.gray:stream-finish-output ((stream synonym-stream))
  (finish-output (follow-synonym-stream stream)))

(defmethod sys.gray:stream-force-output ((stream synonym-stream))
  (force-output (follow-synonym-stream stream)))

(defmethod sys.gray:stream-write-sequence ((stream synonym-stream) seq &optional (start 0) end)
  (write-sequence seq (follow-synonym-stream stream) :start start :end end))

(defmethod sys.gray:stream-read-byte ((stream synonym-stream))
  (read-byte (follow-synonym-stream stream) nil :eof))

(defmethod sys.gray:stream-write-byte ((stream synonym-stream) integer)
  (write-byte integer (follow-synonym-stream stream)))

(defmethod sys.gray:stream-peek-char ((stream synonym-stream))
  (peek-char nil (follow-synonym-stream stream) nil :eof))

(defmethod sys.gray:stream-read-char-no-hang ((stream synonym-stream))
  (read-char-no-hang (follow-synonym-stream stream) nil :eof))

(defmethod sys.gray:stream-read-char ((stream synonym-stream))
  (read-char (follow-synonym-stream stream) nil :eof))

(defmethod sys.gray:stream-read-line ((stream synonym-stream))
  (read-line (follow-synonym-stream stream) nil ""))

(defmethod sys.gray:stream-listen ((stream synonym-stream))
  (listen (follow-synonym-stream stream)))

(defmethod sys.gray:stream-unread-char ((stream synonym-stream) character)
  (unread-char character (follow-synonym-stream stream)))

(defmethod sys.gray:stream-advance-to-column ((stream synonym-stream) column)
  (advance-to-column column (follow-synonym-stream stream)))

(defmethod sys.gray:stream-fresh-line ((stream synonym-stream))
...
(defmethod sys.gray:stream-start-line-p ((stream synonym-stream))
  (start-line-p (follow-synonym-stream stream)))

(defmethod sys.gray:stream-terpri ((stream synonym-stream))
  (terpri (follow-synonym-stream stream)))

(defmethod sys.gray:stream-write-char ((stream synonym-stream) character)
  (write-char character (follow-synonym-stream stream)))

(defmethod sys.gray:stream-write-string ((stream synonym-stream) string &optional (start 0) end)
  (write-string string (follow-synonym-stream stream) :start start :end end))

(defmethod sys.gray:stream-display ((stream synonym-stream) object)
  (sys.gray:stream-display (follow-synonym-stream stream) object))

;;; Broadcast stream.

(defclass broadcast-stream (sys.gray:fundamental-output-stream)
...
(defmethod sys.gray:stream-write-sequence ((stream broadcast-stream) seq &optional (start 0) end)
  (broadcast-stream-op (substream stream start)
    (write-sequence seq substream :start start :end end)))

(defmethod sys.gray:stream-element-type ((stream broadcast-stream))
  (broadcast-stream-op (substream stream t)
    (stream-element-type substream)))

(defmethod sys.gray:stream-fresh-line ((stream broadcast-stream))
  (broadcast-stream-op (substream stream nil)
    (fresh-line substream)))

(defmethod sys.gray:stream-file-length ((stream broadcast-stream))
  (broadcast-stream-op (substream stream 0)
    (file-length substream)))

(defmethod sys.gray:stream-file-string-length ((stream broadcast-stream) string)
  (broadcast-stream-op (substream stream 1)
    (file-string-length substream)))

(defmethod sys.gray:stream-file-position ((stream broadcast-stream) &optional (position-spec nil position-spec-p))
  (if position-spec-p
      (broadcast-stream-op (substream stream nil)
...
(defmethod sys.gray:stream-line-column ((stream broadcast-stream))
  (broadcast-stream-op (substream stream nil)
    (line-column substream)))

(defmethod sys.gray:stream-line-length ((stream broadcast-stream))
  (broadcast-stream-op (substream stream nil)
    (line-length substream)))

(defmethod sys.gray:stream-advance-to-column ((stream broadcast-stream) column)
  (broadcast-stream-op (substream stream nil)
    (advance-to-column column substream)))

(defmethod sys.gray:stream-clear-output ((stream broadcast-stream))
  (broadcast-stream-op (substream stream nil)
    (clear-output substream)))

(defmethod sys.gray:stream-finish-output ((stream broadcast-stream))
  (broadcast-stream-op (substream stream nil)
    (finish-output substream)))

(defmethod sys.gray:stream-force-output ((stream broadcast-stream))
  (broadcast-stream-op (substream stream nil)
    (force-output substream)))

(defmethod sys.gray:stream-start-line-p ((stream broadcast-stream))
  (broadcast-stream-op (substream stream nil)
    (start-line-p substream)))

(defmethod sys.gray:stream-terpri ((stream broadcast-stream))
  (broadcast-stream-op (substream stream nil)
    (terpri substream)))

(defmethod sys.gray:stream-write-string ((stream broadcast-stream) string &optional (start 0) end)
...
(defclass echo-stream (sys.gray:fundamental-output-stream
                       sys.gray:fundamental-input-stream
...
(defmethod sys.gray:stream-write-byte ((stream echo-stream) character)
...
(defmethod sys.gray:stream-read-byte ((stream echo-stream))
...
(defmethod sys.gray:stream-read-char-no-hang ((stream echo-stream))
  (let ((c (read-char-no-hang (echo-stream-input-stream stream) nil :eof)))
    (when (characterp c)
      (write-char c (echo-stream-output-stream stream)))
    c))

(defmethod sys.gray:stream-read-sequence ((stream echo-stream) seq &optional (start 0) end)
...
(defmethod sys.gray:stream-listen ((stream echo-stream))
  (listen (echo-stream-input-stream stream)))

(defmethod sys.gray:stream-clear-input ((stream echo-stream))
  (clear-input (echo-stream-input-stream stream)))

(defmethod sys.gray:stream-clear-output ((stream echo-stream))
  (clear-output (echo-stream-output-stream stream)))

(defmethod sys.gray:stream-finish-output ((stream echo-stream))
  (finish-output (echo-stream-output-stream stream)))

(defmethod sys.gray:stream-force-output ((stream echo-stream))
  (force-output (echo-stream-output-stream stream)))

(defmethod sys.gray:stream-peek-char ((stream echo-stream))
  ;; Don't echo when peeking.
  (peek-char nil (echo-stream-input-stream stream) nil :eof))

(defmethod sys.gray:stream-read-line ((stream echo-stream))
...
(defmethod sys.gray:stream-fresh-line ((stream echo-stream))
  (fresh-line (echo-stream-output-stream stream)))

(defmethod sys.gray:stream-line-column ((stream echo-stream))
  (line-column (echo-stream-output-stream stream)))

(defmethod sys.gray:stream-advance-to-column ((stream echo-stream) column)
  (advance-to-column column (echo-stream-output-stream stream)))

(defmethod sys.gray:stream-line-length ((stream echo-stream))
  (line-length (echo-stream-output-stream stream)))

(defmethod sys.gray:stream-start-line-p ((stream echo-stream))
  (start-line-p (echo-stream-output-stream stream)))

(defmethod sys.gray:stream-terpri ((stream echo-stream))
  (terpri (echo-stream-output-stream stream)))

(defmethod sys.gray:stream-write-string ((stream echo-stream) string &optional (start 0) end)
  (write-string string (echo-stream-output-stream stream) :start start :end end))

;;; Two-way stream.

(defclass two-way-stream (sys.gray:fundamental-output-stream
                          sys.gray:fundamental-input-stream
                          sys.gray:unread-char-mixin)
  ((input-stream :initarg :input-stream
...
(defmethod sys.gray:stream-write-byte ((stream two-way-stream) byte)
...
(defmethod sys.gray:stream-write-sequence ((stream two-way-stream) seq &optional (start 0) end)
  (write-sequence seq (two-way-stream-output-stream stream) :start start :end end))

(defmethod sys.gray:stream-read-byte ((stream two-way-stream))
...
(defmethod sys.gray:stream-read-char-no-hang ((stream two-way-stream))
  (read-char-no-hang (two-way-stream-input-stream stream) nil :eof))

(defmethod sys.gray:stream-read-sequence ((stream two-way-stream) seq &optional (start 0) end)
  (read-sequence seq (two-way-stream-input-stream stream) :start start :end end))

(defmethod sys.gray:stream-listen ((stream two-way-stream))
  (listen (two-way-stream-input-stream stream)))

(defmethod sys.gray:stream-clear-input ((stream two-way-stream))
  (clear-input (two-way-stream-input-stream stream)))

(defmethod sys.gray:stream-clear-output ((stream two-way-stream))
  (clear-output (two-way-stream-output-stream stream)))

(defmethod sys.gray:stream-finish-output ((stream two-way-stream))
  (finish-output (two-way-stream-output-stream stream)))

(defmethod sys.gray:stream-force-output ((stream two-way-stream))
  (force-output (two-way-stream-output-stream stream)))

(defmethod sys.gray:stream-peek-char ((stream two-way-stream))
  (peek-char nil (two-way-stream-input-stream stream) nil :eof))

(defmethod sys.gray:stream-read-line ((stream two-way-stream))
  (read-line (two-way-stream-input-stream stream) nil ""))

(defmethod sys.gray:stream-fresh-line ((stream two-way-stream))
  (fresh-line (two-way-stream-output-stream stream)))

(defmethod sys.gray:stream-line-column ((stream two-way-stream))
  (line-column (two-way-stream-output-stream stream)))

(defmethod sys.gray:stream-advance-to-column ((stream two-way-stream) column)
  (advance-to-column column (two-way-stream-output-stream stream)))

(defmethod sys.gray:stream-line-length ((stream two-way-stream))
  (line-length (two-way-stream-output-stream stream)))

(defmethod sys.gray:stream-start-line-p ((stream two-way-stream))
  (start-line-p (two-way-stream-output-stream stream)))

(defmethod sys.gray:stream-terpri ((stream two-way-stream))
  (terpri (two-way-stream-output-stream stream)))

(defmethod sys.gray:stream-write-string ((stream two-way-stream) string &optional (start 0) end)
  (write-string string (two-way-stream-output-stream stream) :start start :end end))

;;; Concatenated stream.

(defclass concatenated-stream (sys.gray:fundamental-character-input-stream ; For the default stream-read-line method.
                               sys.gray:fundamental-input-stream
                               sys.gray:unread-char-mixin)
...
(defmethod sys.gray:stream-read-sequence ((stream concatenated-stream) seq &optional (start 0) end)
...
(defmethod sys.gray:stream-read-byte ((stream concatenated-stream))
  (loop
     (when (endp (concatenated-stream-streams stream))
       (return :eof))
...
(defmethod sys.gray:stream-read-char-no-hang ((stream concatenated-stream))
  (loop
     (when (endp (concatenated-stream-streams stream))
       (return :eof))
...
(defmethod sys.gray:stream-listen ((stream concatenated-stream))
...
(defmethod sys.gray:stream-clear-input ((stream concatenated-stream))
  (if (concatenated-stream-streams stream)
      (clear-input (first (concatenated-stream-streams stream)))
      nil))

(defmethod sys.gray:stream-peek-char ((stream concatenated-stream))
  (loop
     (when (endp (concatenated-stream-streams stream))
       (return :eof))
...
(defmethod sys.gray:stream-start-line-p ((stream string-output-stream))
...
(defmethod sys.gray:stream-read-sequence ((stream string-input-stream) seq &optional (start 0) end)
