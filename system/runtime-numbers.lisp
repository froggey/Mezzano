...
(define-lap-function %single-float-as-integer ()
  ...
  (sys.lap-x86:shl64 :rax #.+n-fixnum-bits+)
...
(define-lap-function %%bignum-left-shift ()
  ...
  (sys.lap-x86:shr64 :rbx #.+array-length-shift+)
  (sys.lap-x86:mov32 :r10d 1)
  loop
  (sys.lap-x86:cmp64 :r10 :rbx)
  (sys.lap-x86:jae last-word)
  (sys.lap-x86:mov64 :rax (:r9 #.(+ (- +tag-object+) 8) (:r10 8)))
  (sys.lap-x86:mov64 :rdx (:r9 #.(+ (- +tag-object+)) (:r10 8)))
  (sys.lap-x86:shld64 :rax :rdx :cl)
  (sys.lap-x86:mov64 (:r8 #.(+ (- +tag-object+) 8) (:r10 8)) :rax)
  (sys.lap-x86:add64 :r10 1)
  (sys.lap-x86:jmp loop)
  last-word
  (sys.lap-x86:mov64 :rax (:r9 #.(- +tag-object+) (:rbx 8)))
  (sys.lap-x86:cqo)
  (sys.lap-x86:shld64 :rdx :rax :cl)
  (sys.lap-x86:mov64 (:r8 #.(+ (- +tag-object+) 8) (:rbx 8)) :rdx)
...
(define-lap-function %%bignum-right-shift ()
  ...
  (sys.lap-x86:jae last-word)
  ;; current+1
  (sys.lap-x86:mov64 :rax (:r9 #.(+ (- +tag-object+) 8) :r10))
  ;; current
  (sys.lap-x86:mov64 :rdx (:r9 #.(- +tag-object+) :r10))
  (sys.lap-x86:shrd64 :rdx :rax :cl)
  (sys.lap-x86:mov64 (:r8 #.(- +tag-object+) :r10) :rdx)
  (sys.lap-x86:add64 :r10 #.(ash 1 +n-fixnum-bits+))
  (sys.lap-x86:jmp loop)
  last-word
  (sys.lap-x86:mov64 :rax (:r9 #.(- +tag-object+) :rbx))
  (sys.lap-x86:sar64 :rax :cl)
  (sys.lap-x86:mov64 (:r8 #.(- +tag-object+) :rbx) :rax)
  (sys.lap-x86:mov32 :ecx #.(ash 1 +n-fixnum-bits+))
  (sys.lap-x86:mov64 :r13 (:constant %%canonicalize-bignum))
  (sys.lap-x86:leave)
  (:gc :no-frame)
