...
(defun general-area-usage ()
  (let ((bump (+ int::*general-area-old-gen-bump* int::*general-area-young-gen-bump*))
        (limit (+ int::*general-area-old-gen-limit* int::*general-area-young-gen-limit*)))
    (/ (float bump) limit)))

(defun general-area-alloc ()
  (+ int::*general-area-old-gen-bump* int::*general-area-young-gen-bump*))

(defun general-area-commit ()
  (+ int::*general-area-old-gen-limit* int::*general-area-young-gen-limit*))

(defun cons-area-usage ()
  (let ((bump (+ int::*cons-area-old-gen-bump* int::*cons-area-young-gen-bump*))
        (limit (+ int::*cons-area-old-gen-limit* int::*cons-area-young-gen-limit*)))
    (/ (float bump) limit)))

(defun cons-area-alloc ()
  (+ int::*cons-area-old-gen-bump* int::*cons-area-young-gen-bump*))

(defun cons-area-commit ()
  (+ int::*cons-area-old-gen-limit* int::*cons-area-young-gen-limit*))

(defun pinned-area-usage ()
  (multiple-value-bind (allocated-words total-words)
      (int::area-info :pinned)
    (/ (float allocated-words) total-words)))
...
(defun function-area-usage ()
  (multiple-value-bind (allocated-words total-words)
      (int::area-info :function)
    (/ (float allocated-words) total-words)))

(defun wired-function-area-usage ()
  (multiple-value-bind (allocated-words total-words)
      (int::area-info :wired-function)
    (/ (float allocated-words) total-words)))
